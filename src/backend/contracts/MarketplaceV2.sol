...
contract MarketplaceV2 is ERC721Holder, Ownable, ReentrancyGuard {
    ...
    struct Item {
        ...
        address coin; // address of the coin
        bool withBNB; // with BNB
    }
    ...
    constructor(
        ...
        adminAddress = 0x902BD16114211f4A66304d52d47e9F7BB5cDA6a9;
        treasuryAddress = 0x902BD16114211f4A66304d52d47e9F7BB5cDA6a9;
    }
     ...
     */
    function cancelItemOrder(address _collection, uint256 _tokenId)
        external
        nonReentrant
    {
     ...
     */
    function claimPendingRevenue(address _coin) external nonReentrant {
        ...
        IERC20(_coin).safeTransfer(address(msg.sender), revenueToClaim);
     ...
     * @param _collection: contract address of the NFT
     * @param _itemPrice: price for listing (in wei)
     */
    ...
    function modifyItemOrder(
        ...
        // Adjust the information
        _itemTokenIds[_collection].add(_tokenId);
        _itemDetails[_collection][_tokenId].price = _newPrice;
    ...
    {
        require(
            !_itemTokenIds[_token].contains(_tokenId),
    ...
    {
        uint256 length =  _tokenIdsOfSellerForCollection[seller][collection].length();
    ...
    /**
     * @notice Buy token with BNB by matching the price of an existing item order
     * @param _collection: contract address of the NFT
        ...
        nonReentrant
    {
        ...
        _tokenIdsOfSellerForCollection[itemOrder.seller][_collection].remove(
            _tokenId
        );
         _itemTokenIds[_collection].remove(_tokenId);
        // Transfer WBNB
        payable(itemOrder.seller).transfer(_price);
        //IERC20(_coin).safeTransfer(itemOrder.seller, netPrice);
    ...
    function buyTokenUsingCoin(
        ...
        // Transfer WBNB
        IERC20(_coin).safeTransferFrom(msg.sender,itemOrder.seller, netPrice);
        itemOrder.seller = msg.sender;
        _itemDetails[_collection][_tokenId] = itemOrder;
        // Update pending revenues for treasury/creator (if any!)
        if (creatorFee != 0) {
    ...
    /**
     * @notice Buy token by matching the price of an existing item order
     * @param _collection: contract address of the NFT
     * @param _tokenId: tokenId of the NFT purchased
     * @param _price: price (must match the itemPrice from the seller)
    ...
    function _buyToken(
        ...
        // Transfer WBNB
        // IERC20(WBNB).safeTransfer(itemOrder.seller, netPrice);
        ...
        )
    {
        ...
        creatorFee = (_itemPrice * _collections[_collection].creatorFee) / 10000;

        netPrice = _itemPrice - tradingFee - creatorFee;
